...
date: 2026-01-08
draft: true
tags: ["Azure", "Agents", "Azure Container Apps", "DevOps", "IaC"]
ShowToc: true
TocOpen: true
cover:
...
*N.B MTTR is the essentially the time taken to restore a component after a failure is detected. For me details, [check the docs](https://learn.microsoft.com/en-us/azure/well-architected/reliability/metrics#define-recovery-metrics).*
...
You can also connect to external services, such as external monitoring and observability systems, incident managemenet systems, source control, data sources, and MCP servers.
...
Once you have all of that, we can create our agent. Like I said earlier, we *currently* can't create an agent using Bicep (I don't know when we will either). We have to do it through the portal.
...
On the positive side, the Agent would be useful for new team members exploring their Azure resources for the first time, or for team members to see the current status of their Azure resources. This is certainly better than having to trawl through resource groups and graph maps to see how everything fits together.
...
So it switched the revision mode back to `Multiple`, which is was able to do, and then it set the traffic back to 100%. Success right?
...
```bash
Process failed with: exit code 1: WARNING: The behavior of this command has been altered by the following extension: containerapp
...
It was adamant to keep the ingress disabled, which isn't ideal for a public facing API 😅 After an hour of struggling with the agent's hullicinations, I just went back into the Container App and fixed the problem myself.
